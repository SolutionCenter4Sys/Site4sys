...
export function CTASection({
  headline = 'Pronto para transformar com resultado?',
  subheadline = 'Fale com nosso time e agende um diagnóstico gratuito.',
  ctaLabel = 'Fale com um especialista',
  ctaHref = '/contato',
  ctaSecondaryLabel = 'Ver nossos casos',
  ctaSecondaryHref = '/casos-de-sucesso',
  variant = 'gradient',
    ...
    <section className={cn('relative overflow-hidden py-20 lg:py-28', bgClass, className)} aria-label="Chamada para ação">
      ...
      <div className="container-site relative z-10 text-center">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-pill bg-white/10 border border-white/20 mb-6">
          ...
          <span className="text-label-md text-white/90 font-semibold">+100 empresas transformadas</span>
        </div>

        <h2 className="text-3xl sm:text-display-sm text-white font-bold mb-4 max-w-2xl mx-auto">
          {headline}
        </h2>
        <p className="text-body-xl text-white/70 mb-8 max-w-xl mx-auto">{subheadline}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button href={ctaHref} variant="white" size="xl" rightIcon={<ArrowRight className="w-5 h-5" />}>
            {ctaLabel}
          </Button>
          {ctaSecondaryLabel && (
            <Button href={ctaSecondaryHref} variant="outline" size="xl" className="border-white/40 text-white hover:bg-white/10">
              {ctaSecondaryLabel}
            </Button>
          )}
        </div>
